...
function sunsetWp_theme_contact(){
	...
	//Generate sunsetWp-theme custom css page template
function sunsetWp_theme_css_page() {
	require_once(get_template_directory().'/inc/admin-templates/sunsetWp-css.php');
}
...
//*Theme custom css*
	...
	//Register custom css Section
	register_setting('sunsetWp-custom-css', 'custom_css', 'sunsetWp_sanitize_editor_input');	

	add_settings_section('sunsetWp-custom-css-section', 'Custom CSS', 'sunsetWp_custom_css_section_callback', 'sunsetWp_css'); //(section-id, title, fieldname, callback, page-url)

	add_settings_field('custom-css', 'Insert Your Custom CSS', 'sunsetWp_custom_css_field_callback', 'sunsetWp_css', 'sunsetWp-custom-css-section');//(id, title, callback, page-url, section-id);	
...
//*Section Settings Callbacks*
	...
	function sunsetWp_custom_css_section_callback() {
		echo "<p>You can add your own CSS here.</p>";
	}
...
function sunsetWp_custom_css_field_callback() {
	$css= get_option('custom_css');

	$css = (empty($css) ? '/*SunsetWp Custom css*/' : $css);

	echo '<div id="editor">'.$css.'</div><textarea id="custom_css" name="custom_css" style="display:none;visibility:hidden;" >'.$css.'</textarea>';
}
...
//sanitize editor textarea  
function sunsetWp_sanitize_editor_input($input) {
